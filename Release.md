...
| **`v4.0.0`** | **`Active / Production`** | [`Release-4.md`](Release-4.md) | **Spark Module**: Sub-10ms native Rust ephemeral thought capture HUD; native `Ctrl+Alt+S` Windows Explorer shortcut (0 MB idle RAM / 0% CPU); Void Black (`#0A0C10`) auto-focus card; smart bullet engine (`- ` / `1. `); atomic append-only stream (`~/.gemini/spark.jsonl`). |
